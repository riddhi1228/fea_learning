...
def main():
	...
	dt = float(config["TIME_STEP"]) * float(options.SamplingFreq)
	SamplingFreq = 1.0
	M_PI = np.pi
	...
	surface_geo        = np.zeros((nPanel,2*options.nDim+3+4*nObserver), dtype = np.float32)
	...
	start_iter = int(config["RESTART_ITER"])
